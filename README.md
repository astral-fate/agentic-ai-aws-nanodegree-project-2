...
[`cloudshell/deploy-e2e-v3.sh`](cloudshell/deploy-e2e-v3.sh) is **self-contained** —
...
```bash
curl -sSL https://raw.githubusercontent.com/astral-fate/agentic-ai-aws-nanodegree-project-2/main/cloudshell/deploy-e2e-v3.sh -o deploy-e2e-v3.sh && bash deploy-e2e-v3.sh
```
...
```bash
bash deploy-e2e-v3.sh --status      # what exists, change nothing
bash deploy-e2e-v3.sh --test-only   # re-run the six tests
bash deploy-e2e-v3.sh --teardown    # delete everything, OpenSearch first
```
...
cloudshell/
  deploy-e2e-v3.sh            ★ self-contained one-command deploy + all six tests
