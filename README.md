...
[`cloudshell/deploy-e2e-v8.sh`](cloudshell/deploy-e2e-v8.sh) is **self-contained** —
...
```bash
bash deploy-e2e-v8.sh --status      # what exists, change nothing
...
cloudshell/
  deploy-e2e-v8.sh            ★ self-contained one-command deploy + all six tests
