...
[`cloudshell/deploy-e2e-v15.sh`](cloudshell/deploy-e2e-v15.sh) is **self-contained** —
...
```bash
curl -sSL https://raw.githubusercontent.com/astral-fate/agentic-ai-aws-nanodegree-project-2/main/cloudshell/deploy-e2e-v15.sh -o deploy-e2e-v15.sh && bash deploy-e2e-v15.sh
```
...
```bash
bash deploy-e2e-v15.sh --status      # what exists, change nothing
bash deploy-e2e-v15.sh --test-only   # re-run the six tests
bash deploy-e2e-v15.sh --teardown    # delete everything, OpenSearch first
```
...
cloudshell/
  deploy-e2e-v15.sh            ★ self-contained one-command deploy + all six tests
